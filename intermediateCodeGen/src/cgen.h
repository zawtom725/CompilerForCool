//
// This is the MP3 and MP4 skeleton cgen header.  As given, it contains only
// basic functionality.  You will need to add members to each of the classes to
// get them to perform their desired functions.  Document your important design
// decisions below.  We should be able to read your documentation and get a
// general overview of how your compiler generates code.  For instance, how does
// your compiler generate structures for classes, how is inheritance modeled,
// how do you handle dynamic binding, etc.
//
...
private:
	...
	// ADD CODE HERE
public:
	// for convenience -> mapping from string to string const operand
	vector<string> map_class_name;
	vector<global_value> map_vtable;

	vector<string> map_string;
	vector<global_value> map_global;
	int get_global_str_index(string input_str);
};
...
private:
	...
	/* Symbol name inherited */

public:
	// do the public one for convenience
	// the full set, including the inherited ones
	vector<string> class_feature_names;
	vector<op_type> class_features; // vtable + {inherited} + {self}
	int get_feature_index(string name);

	vector<string> class_vtable_names;
	vector<op_func_type> class_vtable; // new + {inherited} + {self}
	vector<op_func_type> inherited_original_vtable;
...
public:
	...
	// ADD CODE HERE
public:
	// added to be used in type_declare
	void declare_class();

	op_type process_type_in_class(string str_type);
};
...
public:
	...
	// keep track of formal
	vector<string> map_formal_string;
	vector<operand> map_formal_operand;
